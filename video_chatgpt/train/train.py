...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in instances[0]:
            # ADDED BY GAURAV
            features = [(torch.tensor(instance['video'][0].clone().detach().requires_grad(True), device="cpu"), 
                         torch.tensor(instance['video'][1].clone().detach().requires_grad(True), device="cpu")) for instance in instances]
