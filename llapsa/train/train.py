...
class DataArguments:
    ...
    mem_folder: Optional[str] = field(default=None)
...
def preprocess_v1(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        cur_len = 1
        target[:cur_len] = IGNORE_INDEX # assign -100 to the first token
        for i, rou in enumerate(rounds):
            ...
            if len(parts) != 2:
                break
            parts[0] += sep   ### parts[0]: command+vis_patch+question; parts[1]: answer
            ...
            cur_len += round_len
        target[cur_len:] = IGNORE_INDEX # mask the tokens after answer length (when batched different length of tokens)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in sources[0]:
            ...
            cur_token_len = features.shape[0] + mem_features.shape[0]
        ...
        if 'video' in self.list_data_dict[i]:
            ...
            data_dict["mem_video"] = mem_features
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'mem_video' in instances[0]:
            ...
            if all(x is not None and x.shape == mem_features[0].shape for x in mem_features):
                ...
                batch['memory_features'] = mem_features
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = dataset_cls(tokenizer=tokenizer,
                                ...
                                multimodal_cfg=dict(
                                    ...
                                    mem_folder=data_args.mem_folder,
