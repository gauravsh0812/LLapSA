...
def main():
    ...
    config.qformer_config.num_attention_heads = 8
    blip_model = Blip2ForConditionalGeneration(config).half().to("cuda")
    ...
    for video_name in tqdm(all_videos):
        ...
        if not os.path.exists(f"{clip_feat_path_local}/{video_id}.pkl"):
            ...
            qformer_output = blip_model.qformer(weighted_features.half()).cuda()
